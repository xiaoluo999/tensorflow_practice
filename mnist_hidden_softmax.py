...
#添加隐藏层
w = tf.Variable(tf.truncated_normal(shape=[784,1024],stddev=0.01))
b = tf.Variable(tf.truncated_normal(shape=[1024],stddev=0.01))
h1 = tf.nn.sigmoid(tf.matmul(x,w)+b)

w1 = tf.Variable(tf.truncated_normal(shape=[1024,10],stddev=0.01))
b1 = tf.Variable(tf.truncated_normal(shape=[10],stddev=0.01))

y_ = tf.nn.softmax(tf.matmul(h1,w1)+b1)
